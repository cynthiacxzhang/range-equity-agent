...
app.post('/api/ai', async (req, res) => {
  if (!API_KEY) {
    return res.status(500).json({ error: 'ANTHROPIC_API_KEY not configured. Copy .env.example to .env and add your key.' });
  }
...
app.listen(PORT, () => {
  console.log(`Poker Agent running at http://localhost:${PORT}`);
  if (!API_KEY) {
    console.log('⚠  No ANTHROPIC_API_KEY found. AI features will be disabled.');
